  ...
  switch (dioError.type) {
    ...
    case DioErrorType.response:
      if (dioError.response != null &&
        ...
        return Failure(dioError.response!.statusCode!, dioError.response!.statusMessage!);
      } else if (dioError.response?.statusCode == ResponseCode.success) {
        return DataSource.success.getFailure();
      } else if (dioError.response?.statusCode == ResponseCode.forbidden) {
...
class ResponseMessage {
  ...
  static String noContent = AppStrings.noContent; //.tr(); // success with no data (no content)
  static String badRequest = AppStrings.badRequest; //.tr(); // failure, API rejected request
  static String unauthorised = AppStrings.unauthorised; //.tr(); // failure, use is not authorised
  static String forbidden = AppStrings.forbidden; //.tr(); //  failure, API rejected request
